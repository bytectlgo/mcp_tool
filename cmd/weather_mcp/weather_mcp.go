...
func main() {
	...
	tool := mcp.NewTool("query",
		...
		mcp.WithString("lang", mcp.Description("返回语言"), mcp.Enum("zh", "en"), mcp.DefaultString("en")),
	)
	wtool := &WeatherTool{apiKey: apiKey}
	srv.AddTool(tool, wtool.handleWeather)
...
func (t *WeatherTool) handleWeather(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	pinyinSlice := pinyin.Pinyin(city, pinyinArgs)

	// 将拼音数组转换为字符串
	var pinyinCity string
	for _, p := range pinyinSlice {
		pinyinCity += strings.Join(p, "")
	}
	...
	// 使用拼音城市名获取天气
	w, err := openweathermap.NewCurrent(units, lang, t.apiKey)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("初始化天气客户端失败: %v", err)), nil
	}

	err = w.CurrentByName(pinyinCity)
	if err != nil {
	...
	if days > 1 {
		...
		oc, err := openweathermap.NewOneCall(units, lang, t.apiKey, nil)
		if err != nil {
